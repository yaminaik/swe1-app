...
class Coveralls:
    ...
    def ensure_token(self):
        ...
        raise CoverallsException(
            ...
            "set the COVERALLS_REPO_TOKEN env var.".format(self.config_filename)
        )
    ...
    def load_config_from_semaphore():
        job = os.environ.get("SEMAPHORE_JOB_UUID") or os.environ.get(  # Classic
            "SEMAPHORE_JOB_ID"
        )  # 2.0
        number = os.environ.get(
            "SEMAPHORE_EXECUTABLE_UUID"
        ) or os.environ.get(  # Classic
            "SEMAPHORE_WORKFLOW_ID"
        )  # 2.0
        pr = os.environ.get("SEMAPHORE_BRANCH_ID") or os.environ.get(  # Classic
        ...
        try:
            ...
            with open(fname) as config:
                try:
                    ...
                    log.warning(
                        "PyYAML is not installed, skipping %s.", self.config_filename
                    )
        except OSError:
            log.debug(
                "Missing %s file. Using only env variables.", self.config_filename
            )
    ...
    def submit_report(self, json_string):
        ...
        try:
            ...
            raise CoverallsException("Could not submit coverage: {}".format(e)) from e
    ...
    def parallel_finish(self):
        ...
        try:
            ...
            raise CoverallsException("Parallel finish failed: {}".format(e)) from e
    ...
    def get_coverage(self):
        ...
        return CoverallReporter(workman, workman.config, base_dir, src_dir).coverage
