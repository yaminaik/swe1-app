...
if MYPY_CHECK_RUNNING:
    ...
    CandidateSortingKey = Tuple[int, int, int, _BaseVersion, BuildTag, Optional[int]]
...
class LinkEvaluator(object):
    ...
    def evaluate_link(self, link):
        ...
        if not version:
            return (
                False,
                "Missing project version for %s" % self.project_name,
            )
            ...
            if is_local_path or is_file_url:
                ...
                if os.path.isdir(path):
                    if expand_dir:
                        ...
                        logger.warning(
                            "Path '{0}' is ignored: " "it is a directory.".format(path),
                        )
    ...
    def find_all_candidates(self, project_name):
        ...
        if file_versions:
            ...
            logger.debug(
                "Local files found: %s",
                ", ".join(
                    [url_to_path(candidate.link.url) for candidate in file_versions]
                ),
            )
...
def _clean_link(url):
    ...
    if result.netloc == "":
        ...
        path = urllib_request.pathname2url(urllib_request.url2pathname(result.path))
    else:
