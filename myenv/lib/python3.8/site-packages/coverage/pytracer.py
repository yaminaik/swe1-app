...
class PyTracer:
    ...
    def _trace(self, frame, event, arg_unused):
        ...
        if (
            self.stopped and sys.gettrace() == self._cached_bound_method_trace
    ...
    def stop(self):
        ...
        if (
            self.threading
            and self.thread.ident != self.threading.current_thread().ident
        ):
        ...
        if self.warn:
            ...
            # has changed to None.
            dont_warn = (
                env.PYPY and env.PYPYVERSION >= (5, 4) and self.in_atexit and tf is None
            )
            if (
                not dont_warn
            ) and tf != self._cached_bound_method_trace:  # pylint: disable=comparison-with-callable
                self.warn(
