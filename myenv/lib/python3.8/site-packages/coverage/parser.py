...
class AstArcAnalyzer:
    ...
    def _handle__Match(self, node):
        ...
        for case in node.cases:
            ...
            if isinstance(case.pattern, ast.MatchAs):
                had_wildcard = True
            self.add_arc(
                last_start, case_start, "the pattern on line {lineno} always matched"
            )
            from_start = ArcStart(
                case_start, cause="the pattern on line {lineno} never matched"
            )
    ...
    def _make_expression_code_method(noun):  # pylint: disable=no-self-argument
        ...
        def _code_object__expression_callable(self, node):
            ...
            self.add_arc(
                start, -start, None, f"didn't finish the {noun} on line {start}"
            )
...
def _is_simple_value(value):
    ...
    return value in [None, [], (), {}, set()] or isinstance(
