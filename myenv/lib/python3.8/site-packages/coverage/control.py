...
class Coverage:
    ...
    def _init(self):
        ...
        set_relative_directory()
        self._file_mapper = (
            relative_filename if self.config.relative_files else abs_file
        )

        # Load plugins
        self._plugins = Plugins.load_plugins(
            self.config.plugins, self.config, self._debug
        )
    ...
    def switch_context(self, new_context):
        ...
        if self._collector.should_start_context:
            self._warn(
                "Conflicting dynamic contexts", slug="dynamic-conflict", once=True
            )
    ...
    def _post_save_work(self):
        ...
        if self._data is not None:
            ...
            for (
                file_path,
                plugin_name,
            ) in self._inorout.find_possibly_unexecuted_files():
    ...
    def xml_report(
        ...
        with override_config(
            ...
            return render_report(
                self.config.xml_output, XmlReporter(self), morfs, self._message
            )
    ...
    def json_report(
        ...
        with override_config(
            ...
            return render_report(
                self.config.json_output, JsonReporter(self), morfs, self._message
            )
    ...
    def lcov_report(
        ...
        with override_config(
            ...
            return render_report(
                self.config.lcov_output, LcovReporter(self), morfs, self._message
            )
        ...
        info = [
            ...
            (
                ...
                human_sorted(
                    ...
                    if (
                        any(slug in k for slug in ("COV", "PY"))
                        or (k in ("HOME", "TEMP", "TMP"))
                    )
                ),
            ),
...
if int(os.environ.get("COVERAGE_DEBUG_CALLS", 0)):  # pragma: debugging
    ...
    Coverage = decorate_methods(show_calls(show_args=True), butnot=["get_data"])(
        Coverage
    )
