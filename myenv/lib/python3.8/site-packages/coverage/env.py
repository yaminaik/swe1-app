...
class PYBEHAVIOR:
    ...
    docstring_only_function = (not PYPY) and (
        (3, 7, 0, "beta", 5) <= PYVERSION <= (3, 10)
    )
    ...
    # (old behavior)?
    trace_decorated_def = (CPYTHON and PYVERSION >= (3, 8)) or (
        PYPY and PYVERSION >= (3, 9)
    )
    ...
    report_absolute_files = (CPYTHON or (PYPYVERSION >= (7, 3, 10))) and PYVERSION >= (
        3,
        9,
    )
...
def debug_info():
    ...
    info = [
        ...
        and name not in {"PYBEHAVIOR", "debug_info"}
        and not isinstance(value, type(os))
    ]
    info += [
        (name, value)
        for name, value in PYBEHAVIOR.__dict__.items()
        if not name.startswith("_")
    ]
    return sorted(info)
